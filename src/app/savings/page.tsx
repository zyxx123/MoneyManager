      ...
      <div className="flex-1 p-4 space-y-4">
        {savings === undefined ? (
          ...
          savings.map(goal => {
            ...
            return (
              <div key={goal.id} className="bg-surface border-[3px] border-black rounded-none p-5 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 hover:-translate-x-0.5 transition-all duration-200">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="font-semibold text-[17px] text-text-main tracking-tight">{goal.name}</h3>
                  <button 
                    onClick={() => handleAddFunds(goal.id)}
                    className="text-[13px] bg-primary/10 text-primary px-3 py-1.5 rounded-none font-semibold hover:bg-primary/20 active:translate-y-1.5 active:translate-x-1.5 active:shadow-none transition-all duration-200"
                  >
                    + Nabung
                  </button>
                </div>
