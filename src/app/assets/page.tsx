    ...
    return (
      <div className="flex flex-col min-h-screen max-w-md mx-auto bg-background text-text-main pb-24">
        <header className="sticky top-0 z-10 flex items-center p-4 bg-background/90 backdrop-blur-md">
          ...
          <h1 className="text-[20px] font-bold ml-2">Tambah Aset Baru</h1>
        </header>
        ...
        <div className="flex-1 p-5">
          <form onSubmit={handleCreate} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[13px] font-bold text-text-secondary uppercase tracking-wider ml-1">Nilai Saat Ini</label>
      ...
      <div className="flex-1 p-4 space-y-4">
        {assets === undefined ? (
          <div className="animate-pulse space-y-4">
            {[1, 2].map(i => <div key={i} className="h-24 bg-surface rounded-2xl" />)}
          </div>
        ) : assets.length === 0 ? (
          <div className="flex flex-col items-center justify-center mt-20 text-text-secondary">
            ...
            <p className="font-bold text-[16px] text-text-main">Belum ada aset</p>
            <p className="text-[14px] mt-1">Mulai catat kekayaan Anda.</p>
          </div>
        ) : (
          assets.map(asset => {
            return (
              <div key={asset.id} className="bg-surface border-[3px] border-black rounded-2xl p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 hover:-translate-x-0.5 transition-all duration-200 flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-[17px] text-text-main tracking-tight">{asset.name}</h3>
