...
export default function ReportsPage() {
  ...
  if (!transactions || !categories) {
    return <div className="p-4 max-w-md mx-auto pt-8 flex justify-center"><div className="animate-pulse w-8 h-8 rounded-none bg-blue-200"></div></div>;
  }
    ...
    <div className="flex flex-col min-h-screen max-w-md mx-auto bg-background text-text-main pb-24">
      ...
      <div className="flex-1 p-4 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-surface rounded-none p-5 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] border-[3px] border-black">
        ...
        <div className="bg-surface rounded-none p-5 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] border-[3px] border-black flex justify-between items-center">
        ...
        <div className="pt-2">
          ...
          {categoryBreakdown.length === 0 ? (
            <div className="bg-surface rounded-none border-[3px] border-black p-8 text-center">
              <p className="text-text-secondary font-medium">Belum ada pengeluaran bulan ini.</p>
            </div>
          ) : (
            <div className="space-y-5">
              {/* Pseudo-chart (horizontal stacked bar) */}
              <div className="h-3 w-full bg-border-subtle/50 rounded-none overflow-hidden flex shadow-[inset_2px_2px_0px_0px_rgba(0,0,0,0.2)]">
              ...
              <div className="bg-surface rounded-none shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] border-[3px] border-black overflow-hidden">
                {categoryBreakdown.map((cat, i) => (
                  <div key={cat.id} className={`flex items-center justify-between p-4 ${i !== categoryBreakdown.length - 1 ? 'border-b-[3px] border-black' : ''} hover:bg-border-subtle/20 transition-colors`}>
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-none flex items-center justify-center text-white shadow-[inset_2px_2px_0px_0px_rgba(0,0,0,0.2)]" style={{ backgroundColor: cat.color }}>
